...
typedef enum { PROGRAM_TAC_NODE, FUNCTION_TAC_NODE,
                RETURN_TAC_NODE, BLOCK_TAC_NODE, INT_CONSTANT_TAC_NODE, UNARY_OP_TAC_NODE, VAR_TAC_NODE, IDENTIFIER_TAC_NODE
} tac_node_type;

bool is_tac_expr(const tac_node_type type);
bool is_tac_stmt(const tac_node_type type);
[[nodiscard]] size_t size_of_tac_node(const tac_node_type type);
