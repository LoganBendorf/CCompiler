...
[[nodiscard]] static expect_asm_node_ptr make_expect_asm_node_ptr(const uint8_t* asm_node_addr) {
...
static register_asm_node* get_return_destination_register() {
...
static expect_asm_node_ptr parse_tac_node(const tac_node_ptr nd) {
    ...
    case PROGRAM_TAC_NODE: {
        ...
        const expect_asm_node_ptr expect = make_expect_asm_node_ptr((const uint8_t*) &asm_program);
    ...
    case FUNCTION_TAC_NODE: {
        ...
        const expect_asm_node_ptr expect = make_expect_asm_node_ptr((const uint8_t*) &fund_asm_nd);
    ...
    case RETURN_TAC_NODE: {
        ...
        const expect_asm_node_ptr expect_expr = parse_tac_node((tac_node_ptr){ret_stmt_nd.expr.addr});
        ...
        const asm_expr_ptr expr = (asm_expr_ptr){expect_expr.node.addr};

        const register_asm_node* src_addr = ((register_asm_node*) expect_src.node.addr);

        const register_asm_node* dst_addr = get_return_destination_register();
        ...
        const expect_asm_node_ptr expect = make_expect_asm_node_ptr((const uint8_t*) &ret_asm_nd);
    ...
    case UNARY_OP_TAC_NODE: {
        ...
        asm_expr_ptr prev={0};
        if (has_prev) {
            const expect_asm_node_ptr expect_prev_expr = parse_tac_node((tac_node_ptr){un_op_tac_nd.prev.addr});
            if (expect_prev_expr.is_error) {
                ERR_RET_EXPECT_ASM_NODE_PTR("Failed to generate code for previous unary op expression"); }
            
            prev = (asm_expr_ptr){expect_prev_expr.node.addr};
        }
        ...
        const expect_asm_node_ptr expect_expr = parse_tac_node((tac_node_ptr){tac_src.addr});
        if (expect_expr.is_error) {
            ERR_RET_EXPECT_ASM_NODE_PTR("Failed to generate code for unary op source"); }

        const expect_asm_node_ptr expect_dst = parse_tac_node(TAC_NODE_TO_TAC_NODE_PTR(tac_dst));
        if (expect_dst.is_error) {
            ERR_RET_EXPECT_ASM_NODE_PTR("Failed to generate code for unary op destination"); }
        const asm_node_type dst_type = (*expect_dst.node.addr);
        if (dst_type != REGISTER_ASM_NODE) {
            ERR_RET_EXPECT_ASM_NODE_PTR("Unary op destination failed to become register"); }


        const asm_expr_ptr       src      = (asm_expr_ptr){expect_expr.node.addr};
        const register_asm_node* dst_addr = (register_asm_node*) expect_dst.node.addr;

        if (!has_prev) {
            const unary_op_asm_node un_op_asm_nd = make_unary_op_asm_node(op_type, src, dst_addr);    

            const expect_asm_node_ptr expect = make_expect_asm_node_ptr((const uint8_t*) &un_op_asm_nd);
            return expect;
        } else {
    ...
    case INT_CONSTANT_TAC_NODE: {
        ...
        const expect_asm_node_ptr expect = make_expect_asm_node_ptr((const uint8_t*) &int_immm_asm_nd);
    ...
    case VAR_TAC_NODE: {
        ...
        const expect_asm_node_ptr expect = make_expect_asm_node_ptr((const uint8_t*) reg_asm_nd_addr);
